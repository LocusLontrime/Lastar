...
class Pathfinder(arcade.Window):
    ...
    def __init__(self, width: int, height: int, a: Point, b: Point, circles: list[Circle]):
        super().__init__(width, height)
        self.start, self.end = a, b
        self.circles = circles
        self.circle_shapes = None
        self.shortest_path = None
        arcade.set_background_color(arcade.color.DUTCH_WHITE)
        self.setup()
        self.g = self.shortest_path_length(self.start, self.end, self.circles)

    def setup(self):
        q = Pathfinder.MULTIPLIER
        dx, dy = Pathfinder.DELTA_X, Pathfinder.DELTA_Y
        self.circle_shapes = arcade.SpriteList()
        for circle in self.circles:
            ...
            self.circle_shapes.append(sprite)

        a = arcade.SpriteCircle(10, arcade.color.GREEN_YELLOW)
        a.center_x, a.center_y = q * self.start.x + dx, q * self.start.y + dy

        b = arcade.SpriteCircle(10, arcade.color.JUNGLE_GREEN)
        b.center_x, b.center_y = q * self.end.x + dx, q * self.end.y + dy

        self.circle_shapes.append(a)
        self.circle_shapes.append(b)
    ...
    def on_draw(self):
        ...
        arcade.start_render()
        # drawing:
        self.circle_shapes.draw()
        # the shortest path visualization:
        for i, vertex in enumerate(sp := self.shortest_path[::-1]):
            if i > 0:
                if vertex.circle == (next_ := sp[i - 1]).circle:
                    min_v, max_v = sorted([vertex, next_], key=lambda x: x.angle)
                    if min_v.rot_dir == RotDir.ANTI_CLOCKWISE:
                        # ARC:
                        arcade.draw_arc_outline(
                            q * vertex.circle.ctr.x + dx,
                            q * vertex.circle.ctr.y + dy,
                            2 * q * vertex.circle.r,
                            2 * q * vertex.circle.r,
                            arcade.color.BLACK,
                            Pathfinder.degrees(min_v.angle),
                            Pathfinder.degrees(max_v.angle),
                            3 + 3
                        )
                    else:
                        # ARCS 1:
                    ...
                    m = 2
                    arcade.draw_line(
                        q * vertex.x + dx + m * (vertex.x - next_.x),
                        q * vertex.y + dy + m * (vertex.y - next_.y),
                        q * next_.x + dx - m * (vertex.x - next_.x),
                        q * next_.y + dy - m * (vertex.y - next_.y),
                        arcade.color.BLACK,
                        3
                    )

    @staticmethod
    def degrees(rads: float):
        return 180 * rads / math.pi

    def shortest_path_length(self, a: Point, b: Point, c: list[Circle], flag=False) -> float:
        ...
        v_hashes = self.build_graph(a, b, circles, flag)
        ...
        self.shortest_path.append(start_vertex)
    ...
    def intersect(p1: Point, p2: Point, circle: Circle, flag: bool = False) -> bool:
        ...
        if p1 != p2:
            ...
            a, b, c = Pathfinder.get_coeffs(p1, p2, flag)
            ...
            if abs(a * circle.ctr.x + b * circle.ctr.y + c) / math.hypot(a, b) < circle.r:  # TODO: What if == ???
                ...
                if Pathfinder.in_(p1, circle) or Pathfinder.in_(p2, circle):
                    return True
            ...
            raise ValueError(f'p1: {p1} and p2: {p2} cannot be the same, method: {Pathfinder.intersect}!')
        return False
    ...
    def get_valid_edges(self, c1_: Circle, c2_: Circle, circles: list[Circle], flag: bool = False) -> list[
        tuple['Vertex', 'Vertex']]:
        ...
        if c1_.r != 0 or c2_.r != 0:
            ...
            angle_c1c2_ox = self.angle_of_intersection(c2, c1)
            ...
            if not self.circle_intersect(c1, c2) and 0 not in [c1.r, c2.r]:
    ...
    def build_graph(self, a: Point, b: Point, circles: list[Circle], flag=False) -> dict:
        ...
        for j in range(len_):
            ...
            for i in range(j + 1, len_):
                if self.circle_intersect(cj := circles_[j], ci := circles_[i]):
                    separators[cj].add(self.angle_of_intersection(circles_[i], circles_[j]))
                    separators[ci].add(self.angle_of_intersection(circles_[j], circles_[i]))
                valid_edges = self.get_valid_edges(circles_[j], circles_[i], circles_, flag)
        ...
        for c in circles_:
            ...
            self.build_edges_circle(verts, seps)
        return v_hashes
    ...
    def validate_ab(self, a: Point, b: Point, circles: list[Circle]) -> bool:
        """checks if a or b point lies in some circle from the list"""
        for circle in circles:
            if self.in_(a, circle) or self.in_(b, circle):
                return False
        return True
    ...
    def get_coeffs(p1: Point, p2: Point, flag: bool = False) -> tuple[float, float, float]:
        ...
        if p1 != p2:
            ...
            raise ValueError(f'method: {Pathfinder.get_coeffs}, p1: {p1} cannot be equal to p2: {p2}')
    ...
    def angle_of_intersection(self, outer_obj: Circle | Point, circle: Circle):
        ...
        if isinstance(outer_obj, Circle):
            ...
            raise ValueError(
                f'method {self.angle_of_intersection} works only for Point or Circle as outer_obj and Circle as circle...')
        sc = self.scalar_product(_x, _y, y_, x_)
...
class Vertex:
    ...
    def edge(self, other: 'Vertex'):
        ...
        if self.circle == other.circle:
            ...
            res = Pathfinder.arc(self.angle, other.angle, self.circle, v1.rot_dir == RotDir.ANTI_CLOCKWISE)
        else:
    ...
    def get_rot_dir(self, twin: 'Vertex'):
        ...
        self._rot_dir = RotDir.CLOCKWISE if Pathfinder.cross_product_val(*l_vector, *r_vector) < 0 else RotDir.ANTI_CLOCKWISE
        ...
        if self != other:
            for circle in circles:
                if circle not in {self.circle, other.circle}:
                    if Pathfinder.intersect(Point(self.x, self.y), Point(other.x, other.y), circle, flag):
                        return False
            return True
        else:
